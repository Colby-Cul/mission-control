...
import { getAgentRunFeed, getUserProfile } from '../lib/queries'
...
export default async function ActivityPage() {
  const [activityLog, auditLog, agentRuns, profile] = await Promise.all([
    getActivityLog(),
    getAuditLog(),
    ...
    <>
      ...
      {activityLog !== null ? (
        ...
        <div style={{ marginBottom: 24 }}>
          <ComingSoon
            title="Activity Log"
            reason="Full event log capturing every action — agent runs, user events, system triggers — with filters by type, actor, and entity."
            icon="📡"
            dataSource="coming-soon:activity_log"
            skeleton="table"
          />
        </div>
      )}
      ...
      {auditLog !== null ? (
        ...
        <ComingSoon
          title="Audit Log"
          reason="Immutable record of all user and system actions for compliance and forensic review."
          icon="🏛️"
